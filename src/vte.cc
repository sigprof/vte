                               ...
                               gsize palette_size)
{
	...
	for (gsize i = 0; i < G_N_ELEMENTS(m_palette); i++) {
		...
		if (i < 16) {
		...
		} else if (i >= VTE_VGA_COLORS_OFFSET && i < VTE_VGA_COLORS_OFFSET + 16) {
			static const uint8_t vga_color_table[16][3] = {
				{ 0x00, 0x00, 0x00 },
				{ 0xaa, 0x00, 0x00 },
				{ 0x00, 0xcc, 0x00 },
				{ 0xaa, 0xaa, 0x00 },
			...
			int j = i - VTE_VGA_COLORS_OFFSET;
			color.red   = vga_color_table[j][0] * 0x101;
			color.green = vga_color_table[j][1] * 0x101;
			color.blue  = vga_color_table[j][2] * 0x101;
		}
		else switch (i) {
			case VTE_DEFAULT_BG:
				if (background) {
